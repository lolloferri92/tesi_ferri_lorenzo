...
In parallelo una configurazione che trova il suo utilizzo nel campo della ricerca musicale e in minor misura nell'ascolto di musica è la \textbf{quadrifonia}; questa tecnica prevede 
...
\begin{equation}
\boldsymbol{g_{n\ scaled}} = \dfrac{\sqrt{\dfrac{C}{r_n^{\prime}-r_0}}\ \boldsymbol{g_n}}{\sqrt{g_1^2 +g_2^2 + g_3^2}}
\label{kkkk}
\end{equation}

così in teoria con il fatto che l'intesità sonora scala come il quadrato della distanza dovremmo avere una giusto riposizionamento dell'oggetto.
...
Nei metadati del format MDA sono presenti gli angoli $\theta, \phi$ e il raggio $\rho$ che identificano la posizione, quello che ci da informazioni sull'altezza è l'angolo (appunto detto di elevazione) $\phi$ 
